...
public class RedisMessageStore implements EventStore {
    ...
    public void save(EventRecord record) {
        try {
            ...
            // 序列化完整的EventRecord对象
            byte[] serializedData = messageSerializer.serialize(record);
            String serializedRecord = Base64.getEncoder().encodeToString(serializedData);
            Map<String, String> eventData = new HashMap<>();
            eventData.put("eventRecord", serializedRecord);
    ...
    public void updateStatus(String eventId, EventStatus status, String errorMessage) {
        try {
            ...
            byte[] serializedData = messageSerializer.serialize(record);
            String serializedRecord = Base64.getEncoder().encodeToString(serializedData);
            redisTemplate.opsForHash().put(key, "eventRecord", serializedRecord);
    ...
    public EventRecord findById(String eventId) {
        try {
            String key = buildEventKey(eventId);
            String serializedRecord = (String) redisTemplate.opsForHash().get(key, "eventRecord");
            
            if (serializedRecord == null) {
                return null;
            }
            
            byte[] serializedData = Base64.getDecoder().decode(serializedRecord);
            return messageSerializer.deserialize(serializedData, EventRecord.class);
